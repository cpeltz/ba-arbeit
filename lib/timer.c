...
ISR(TIMER1_COMPC_vect)
{
  ...
  if (flagLocal_Read(&timer_trigger, TIMER_T_TRIGGER_L))
    {
      // linker T_Trigger aktiv
      if (timer_t_trigger_counter[0] == 0)
        {
          flag_Set(T_TRIGGER_L);
          flagLocal_Clear(&timer_trigger, TIMER_T_TRIGGER_L);
        }
      else
        {
  ...
  if (flagLocal_Read(&timer_trigger, TIMER_T_TRIGGER_R))
    {
      // rechter T_Trigger aktiv
      if (timer_t_trigger_counter[1] == 0)
        {
          flag_Set(T_TRIGGER_R);
          flagLocal_Clear(&timer_trigger, TIMER_T_TRIGGER_R);
        }
      else
        {
...
ISR(TIMER1_OVF_vect)
{
  ...
  for (uint8_t led_counter = 0; led_counter < 5; led_counter++)
    {
...
int8_t
wheel_ReadSpeed(const uint8_t wheel)
{
  switch (wheel)
    {
    case 0:
...
void
trigger_Set_T(const uint8_t wheel, const uint16_t time)
{
  switch (wheel)
    {
    case 0:
    case 1:
      ...
      flagLocal_Set(&timer_trigger, TIMER_T_TRIGGER_L + wheel);
...
void
trigger_Clear_T(const uint8_t wheel)
{
  switch (wheel)
    {
    case 0:
    case 1:
      flagLocal_Clear(&timer_trigger, TIMER_T_TRIGGER_L + wheel);
      flag_Clear(T_TRIGGER_L + wheel);
      break;
    case 2:
      trigger_Clear_T(0);
...
uint16_t
trigger_Get_T(const uint8_t wheel)
{
  uint16_t ret = 0;
  switch (wheel)
    {
