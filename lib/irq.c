...
int16_t
wheel_ReadPosition(uint8_t wheel)
{
  switch (wheel)
    {
    case 0:
...
void
wheel_ClearPosition(uint8_t wheel)
{
  sreg = SREG;
  cli();
  switch (wheel)
...
int16_t
wheel_ReadDifference(void)
{
  return irq_WheelDifference;
}

void
wheel_WriteDifference(int16_t difference)
{
  sreg = SREG;
  cli();
...
void
trigger_Set_P(const uint8_t wheel, const int16_t trigger_position)
{
  sreg = SREG;
  cli();
  switch (wheel)
...
void
trigger_Clear_P(const uint8_t wheel)
{
  sreg = SREG;
  cli();
  switch (wheel)
